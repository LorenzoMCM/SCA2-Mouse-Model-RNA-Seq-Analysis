---
title: "DEG_Enrichment_Test_WGCNA"
author: "Lorenzo.MCM"
date: "2024-09-18"
...
## Fisher's test to check WGCNA modules for DEG content, these require the output from both WGCNA.Rmd files. Only modules with significant results for TG-WT binary variable

```{r}
sigList = lapply(reList, function(df){
  df[df$padj < 0.05,]
})
 
sigLFCList = lapply(sigList, function(df){
  
  df = df[df$log2FoldChange > 1 | df$log2FoldChange < -1,]
  
  df = df[!is.na(df$Gene_Symbol),]
})
 
DEG_36 = unique(c(sigLFCList$res1F_WT36$Gene_Symbol,sigLFCList$res1M_WT36$Gene_Symbol, sigLFCList$res2F_WT36$Gene_Symbol, sigLFCList$res2M_WT36$Gene_Symbol, sigLFCList$res3F_WT36$Gene_Symbol, sigLFCList$res3M_WT36$Gene_Symbol))
 
DEG_41 = unique(c(sigLFCList$res1F_WT41$Gene_Symbol,sigLFCList$res1M_WT41$Gene_Symbol, sigLFCList$res2F_WT41$Gene_Symbol, sigLFCList$res2M_WT41$Gene_Symbol, sigLFCList$res3F_WT41$Gene_Symbol, sigLFCList$res3M_WT41$Gene_Symbol))
```
 
```{r}
grey36 = read.csv("greyMod36.csv")
grey36 = grey36$rownames.kWithin_sorted.
 
green36 = read.csv("greenMod36.csv")
green36 = green36$rownames.kWithin_sorted.

blue36 = read.csv("blueMod36.csv")
blue36 = blue36$rownames.kWithin_sorted.
 
turquoise36 = read.csv("turquoiseMod36.csv")
turquoise36 = turquoise36$rownames.kWithin_sorted.
 
pink36 = read.csv("pinkMod36.csv")
pink36 = pink36$rownames.kWithin_sorted.
 
yellow36 = read.csv("yellowMod36.csv")
yellow36 = yellow36$rownames.kWithin_sorted.
 
modules36 = list(grey = grey36, green = green36, blue = blue36, turquoise = turquoise36, pink = pink36, yellow = yellow36)
```
 
```{r}
magenta41 = read.csv("magentaMod41.csv")
magenta41 = magenta41$rownames.kWithin_sorted.
 
black41 = read.csv("blackMod41.csv")
black41 = black41$rownames.kWithin_sorted.
 
purple41 = read.csv("purpleMod41.csv")
purple41 = purple41$rownames.kWithin_sorted.
 
brown41 = read.csv("brownMod41.csv")
brown41 = brown41$rownames.kWithin_sorted.
 
turquoise41 = read.csv("turquoiseMod41.csv")
turquoise41 = turquoise41$rownames.kWithin_sorted.
 
modules41 = list(magenta = magenta41, black = black41, purple = purple41, brown = brown41, turquoise = turquoise41)
```

### Fisher's Test function and enrichment for DEGs with an absolute LFC>1.0

```{r}
universe36 = union(DEG_36, unlist(modules36))
 
universe41 = union(DEG_41, unlist(modules41))
 
 
enrichment_test = function(modules, DEGs, universe){
   
  num_DEGs_in_module = length(intersect(modules, DEGs))
   
  num_nonDEGs_in_module = length(modules) - num_DEGs_in_module
  
  num_DEGs_not_in_module = length(DEGs) - num_DEGs_in_module
  
  num_nonDEGs_not_in_module = length(universe) - length(DEGs) - num_nonDEGs_in_module
  
  contingencyTable = matrix(
    c(num_DEGs_in_module, num_DEGs_not_in_module,
      num_nonDEGs_in_module, num_nonDEGs_not_in_module),
    nrow = 2
  )
   
  fisher.test(contingencyTable, alternative = "greater")$p.value
   
}
 
enrichment36 = sapply(modules36, enrichment_test, DEGs = DEG_36, universe = universe36)
enrichment36
```
 ### Out of these only blue and green are relevant, when compared to the heatmap yielded by the WGCNA of L36
```{r}
enrichment41 = sapply(modules41, enrichment_test, DEGs = DEG_41, universe = universe41)
enrichment41
```

### Test for absolute LFC > 2.0
