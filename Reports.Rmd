...
```{r eval=FALSE}
BiocManager::install(c('Glimma', 'DESeq2', 'AnnotationDbi', 'regionReport', 'ReportingTools', 'clusterProfiler' , 'select'))
```
...
# Naming rows in the rbps object after their Gene symbol and removing all columns except those with the counts for each condition
...
glimmaVolcano(x = dds3F_WTG, counts = T3F_WTG, groups = c3F_WTG$l3F_WTG)

glimmaVolcano(x = dds2F_WTG, counts = T2F_WTG, groups = c2F_WTG$l2F_WTG)
