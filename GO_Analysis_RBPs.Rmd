...
## Importing panther-db list of RBPs and tidying it up (browsed for RNA binding Proteins and filtered for Mus musculus, exported to and edited in excel so the only row was MGI IDs, removed genes that did not have MGI IDs)

```{r message=FALSE, warning=FALSE}

# Importing list of RBPs, omitting NAs

#mainPanther = as.data.frame(read_excel('rna_met.xlsm', col_names = FALSE))
...
go_PDE = function(x, lfcThreshold = 2, padjThreshold = 0.05, cutOff = 0.05, ont = "BP"){
  
  selectGenes = unique(x$Gene_ID[(x$log2FoldChange > lfcThreshold) & (x$padj < padjThreshold)])
...
go_NDE = function(x, lfcThreshold = -2, padjThreshold = 0.05, cutOff = 0.05, ont = "BP"){
...
sigGenes = function(x, lfcThreshold = 0.5, padjThreshold = 0.05){
...
## Comparing WT and TG mice (positively and negatively DE genes and then all) (these take ~5 minutes to complete per chunk, depending on your CPU single core performance)
...
# Function for KEGG pathways analysis, if output == TRUE it outputs images of the relevant pathways with highlighted DEGs

keggPathAnal = function(x, top = 10, output = FALSE) {

  # Extract and name fold changes
  x = x[(x$log2FoldChange > 0.5 | x$log2FoldChange < -0.5) & x$padj < 0.5,]
  ...
  # Return a df with the names of top 10 pathways, for easy Rmd visualization
...
### WT v All TG
```{r}
keggPathAnal(reList$res1F_WT36, output = TRUE)
...
```{r}
keggPathAnal(reList$res2F_WT36, output = TRUE)
```

```{r}
keggPathAnal(reList$res2M_WT36, output = TRUE)
```

```{r}
keggPathAnal(reList$res3F_WTG, output = TRUE)
```
...
```{r}
keggPathAnal(reList$res3F_WT36, output = TRUE)
```

```{r}
keggPathAnal(reList$res3M_WT36, output = TRUE)
```

### WT v 41

```{r}
keggPathAnal(reList$res1F_WT41, output = TRUE)
```

```{r}
keggPathAnal(reList$res1M_WT41, output = TRUE)
```

```{r}
keggPathAnal(reList$res2F_WT41, output = TRUE)
```

```{r}
keggPathAnal(reList$res2M_WT41, output = TRUE)
```

```{r}
keggPathAnal(reList$res3F_WT41, output = TRUE)
```
...
```{r}
keggPathAnal(reList$resF_36_21, output = TRUE)
```

```{r}
keggPathAnal(reList$resM_36_21,output = TRUE)
```

```{r}
keggPathAnal(reList$resF_36_31, output = TRUE)
```

```{r}
keggPathAnal(reList$resM_36_31, output = TRUE)
```

```{r}
keggPathAnal(reList$resF_36_32, output = TRUE)
```

```{r}
keggPathAnal(reList$resM_36_32, output = TRUE)
```

### 41

```{r}
keggPathAnal(reList$resF_41_21, output = TRUE)
```

```{r}
keggPathAnal(reList$resM_41_21, output = TRUE)
```

```{r}
keggPathAnal(reList$resF_41_31, output = TRUE)
```

```{r}
keggPathAnal(reList$resM_41_31, output = TRUE)
```

```{r}
keggPathAnal(reList$resF_41_32, output = TRUE)
```

```{r}
keggPathAnal(reList$resM_41_32, output = TRUE)
```