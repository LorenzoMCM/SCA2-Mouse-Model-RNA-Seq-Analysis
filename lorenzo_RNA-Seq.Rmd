...
```{r eval=FALSE}
BiocManager::install(c('DESeq2', 'AnnotationDbi', 'vsn', 'org.Mm.eg.db', 'apeglm', 'ReportingTools', 'clusterProfiler' , 'select'))
```
...
# Function for replacing repeats for "NA"
...
# Function for tidying up output
outputTidy = function(x, title = "output.csv"){
  
  df = as.data.frame(do.call(rbind, x))
  
  df = as.data.frame(t(apply(df, 1, replace_repeats_with_na)))
...
outputTidy(genesAll, title = "DEG_all.csv")
