...
#comment this for analyzing all genes (uncomment following line)
rbps = reads[reads$MGI %in% mainPanther,]

#comment this for analyzing only rbps (uncomment last line) 
#rbps = reads
...
keggPathAnalup = function(x) {
  # Check that the input data has the necessary columns
  if(!all(c("log2FoldChange", "Gene_ID") %in% colnames(x))) {
    stop("Input data frame must contain 'log2FoldChange' and 'Gene_ID' columns.")
  }
  ...
  # Create new directory with the name of the input object `x`
  output_dir = as.character(deparse(substitute(x)))
  ...
  sapply(keggIDs, function(pid) {
    tryCatch({
      pathview(gene.data = foldchanges, pathway.id = pid, species = "mmu", out.suffix = output_dir, kegg.dir = output_dir)
    }, error = function(e) {
   ...
   files_to_move <- list.files(pattern = paste0(output_dir, ".*"))
   file.rename(files_to_move, file.path(output_dir, files_to_move))
}
