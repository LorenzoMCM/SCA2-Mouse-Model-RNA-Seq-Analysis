...
#MGIrbps = as.character(rbps$Gene_ID)

#rbps$Gene_ID = mapIds(org.Mm.eg.db,MGIrbps,keytype = "ENTREZID","ENSEMBL")

#rbps = na.omit(rbps)

#rbps <- rbps %>%
#  add_count(Gene_ID) %>%        # Add a count of Gene_ID occurrences
#  filter(n == 1) %>%            # Keep only rows where count is 1
...
degenes = function(x, padjThreshold = 0.05, max = 20, key = "Gene_Symbol") {
  
  # Filter rows based on adj
  filtered_x <- x[(x$padj < padjThreshold) & !is.na(x$padj),]
...
genespAll = lapply(reList, pdegenes, padjThreshold = 0.1, max = 20)

genesnAll = lapply(reList, ndegenes, padjThreshold = 0.1, max = 20)

genesAll = lapply(reList, degenes, padjThreshold = 0.1, max = 20)
```
...
```{r}
posfilter = function(x, lfcThreshold = 2, padjThreshold = 0.05){
  
  obj_name = deparse(substitute(x))
  
  new_obj_name = paste0("pos_", obj_name)
  
  df = as.data.frame(x)
  
#  df$MGI <- rownames(df)
  
#  df <- merge(df, reads[, c("MGI", "Gene_Symbol")], by = "MGI", all.x = TRUE)
  
  dfsub = subset(df, padj <= padjThreshold & log2FoldChange > lfcThreshold & !is.na(padj))
  
  rownames(dfsub) = NULL

  assign(new_obj_name, dfsub, envir = .GlobalEnv)
  
}

negfilter = function(x, lfcThreshold = -2, padjThreshold = 0.05){
  
  obj_name = deparse(substitute(x))
  
  new_obj_name = paste0("neg_", obj_name)
...
```{r}
#posList = lapply(reList, posfilter, lfcThreshold = 1, padjThreshold = 0.1)
...
#goDHelpMe = function(x){

#          universeGenes = unique(x[["Gene_ID"]])
#
...
#clusterExport(cl, "goDHelpMe")

```

```{r}
#what = parLapply(cl,posList,goDHelpMe)
```
